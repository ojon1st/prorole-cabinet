...
exports.my_agenda_get = function(req, res, next) {
  res.render('agenda/my_agenda', { title: 'Personnel'});
};

exports.renvois_my_events_get = function(req, res, next){
  async.parallel({
    ...
    last_renvois: function(callback){
      Instruction.find({}, {'renvois':{'$slice':-1},'_id':1,'dossier':1,decision:1})
        .populate('dossier')  
        .exec(callback);
    }
      ...
      results.last_renvois.forEach(function(last_renvoi){
        if(last_renvoi.renvois.length > 0 && moment().diff(moment(last_renvoi.renvois[0].r_date), 'days') > 0 && (last_renvoi.dossier.titulaire.toString() === req.user._id.toString() || (last_renvoi.dossier.attributaire && last_renvoi.dossier.attributaire.toString() === req.user._id.toString()))){
          if(!last_renvoi.decision || last_renvoi.decision == ""){
            //console.log('test last renvoi reussi')
      ...
      results.instructions.forEach(function(instruction){ // Pour chaque instruction
        if (instruction.renvois.length > 0 && (instruction.dossier.titulaire.toString() === req.user._id.toString() || (instruction.dossier.attributaire && instruction.dossier.attributaire.toString() === req.user._id.toString()))){ // Si renvoi il y a
